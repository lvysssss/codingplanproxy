...
- **动态模型选择** — 请求中指定 `model` 字段时使用请求值，否则使用配置的默认模型；仅允许 `AVAILABLE_MODELS` 白名单内的模型
...
| `AVAILABLE_MODELS` | 否 | `claude-sonnet-4-20250514` | 可用模型白名单（逗号分隔），请求的 model 必须在此列表中 |
...
API_KEY=sk-ant-api03-your-key-here
AVAILABLE_MODELS=claude-sonnet-4-20250514,claude-opus-4-20250514,claude-haiku-4-20250514
...
请求中指定 `model` 字段可覆盖默认模型，无需修改配置。**注意**：`model` 必须在 `AVAILABLE_MODELS` 白名单中，否则返回 400 错误。
...
# 失败 — model 不在白名单内，返回 400
# {"detail": "不支持的模型: gpt-4，可用模型: claude-sonnet-4-20250514, claude-opus-4-20250514"}
```
...
请求中未指定 `model` 或指定的 `model` 不在 `AVAILABLE_MODELS` 白名单中。解决方案：
- 在请求体中携带 `model` 字段
- 在 `.env` 中将目标模型加入 `AVAILABLE_MODELS`（逗号分隔）
- 或使用 `/v1/models` 端点查看当前可用模型列表
